 ...
 *
 * EE server stores EE course information
 *****************************************************************************/
...
{
    // attributes
    map<string,vector<string> > eeMap;
    ...
    public:
        ...
        {
            // boot up message
            cout << "The ServerEE is up and running using UDP on port " << SERVERPORT << "." << endl;
            title['1'] = "Credits";
            title['2'] = "Professor";
            title['3'] = "Days";
            title['4'] = "Course Name";
        ...
        string type1(int len) {
            ...
            cout << "The ServerEE received a request from the Main Server about the " << title[attr] << " of " << course << "." << endl;
            // printMap(eeMap);
            if(eeMap.count(course) == 0) {
                // when course is not found
                cout << "Didn’t find the course: " << course << "." << endl;
                return "0";
            }
            cout << "The course information has been found: The " << title[attr] << " of " << course << " is " << eeMap[course][attr - '1'] << "." << endl;
            string reMess = "1," + eeMap[course][attr - '1'];
            return reMess;
        }
        ...
        string type2(int len) {
            ...
            vector<string> temp = eeMap[course];
            cout << "Course is " << course << endl;
    ...
    private:
        ...
        void initialUDPSock() {
            ...
            int addrLen = sizeof(addrEE);
            addrEE.sin_family = AF_INET;
            inet_aton(LOCAL_BIND_IP, &addrEE.sin_addr); 
            addrEE.sin_port = htons(SERVERPORT);
            if(bind(udpSock, (sockaddr *)&addrEE, addrLen) < 0) {
